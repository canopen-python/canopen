...
class TestEDS(unittest.TestCase):
    ...
    test_data = {
        ...
        "int64": [
            {
                "hex_str": "7FFFFFFFFFFFFFFF",
                "bit_length": 64,
                "expected": 9223372036854775807,
            },
            {
                "hex_str": "8000000000000000",
                "bit_length": 64,
        ...
        for data_type, test_cases in self.test_data.items():
            for test_case in test_cases:
                with self.subTest(data_type=data_type, test_case=test_case):
                    result = _signed_int_from_hex(
                        "0x" + test_case["hex_str"], test_case["bit_length"]
                    )
        ...
        for index in self.od:
            ...
            for evar, avar in zip(expected_vars, actual_vars):
                ...
                self.assertEqual(
                    ...
                    getattr(evar, "max", None),
                    f" mismatch on {pretty_index(evar.index, evar.subindex)}",
                )
                if doctype == "dcf":
                    self.assertEqual(
                        getattr(avar, "value", None),
                        getattr(evar, "value", None),
                        f" mismatch on {pretty_index(evar.index, evar.subindex)}",
                    )
